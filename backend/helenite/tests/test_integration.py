...
def test_unlike(db, create_new_user, valid_data_for_post) -> None:
    """
    ...
    found = Like.objects.get(like_owner=create_new_user, like_parent_post=new_post)
    found.delete()

    assert Like.objects.count() == 0
    assert len(Like.objects.filter(like_owner=create_new_user)) == 0
...
def test_delete_comment(db, valid_data_for_post, valid_data_for_comment, create_new_user) -> None:
    """
    ...
    found = Comment.objects.filter(comment_user=create_new_user.id, comment_parent_post=new_post)
    found.delete()
...
def test_generic_new_friend(db, create_new_user, valid_data_for_user_and_profile) -> None:
    """
    ...
    new_user_2 = User.objects.create(username="test2", email="email@myemail.com", password="dfhsjkalf6789")
...
def test_multiple_friends(db, create_new_user, valid_data_for_user_and_profile) -> None:
    ...
    new_user_3 = User.objects.create(username="test3", email="email@myemail.com", password="dfhsjkalf6789")
    valid_data_for_user_and_profile["user"] = new_user_3
    valid_data_for_user_and_profile["custom_slug_profile"] = "test3"
    friends.append(Profile.objects.create(**valid_data_for_user_and_profile))

    new_user_4 = User.objects.create(username="test4", email="email@myemail.com", password="dfhsjkalf6789")
